...
@testset "DisjointSets.jl" begin
        ...
        for T in [Int, UInt8, Int8, UInt16, Int16, UInt32, Int32, UInt64]
            @testset "eltype = $(T)" begin
                ...
                @testset "basic tests" begin
                    ...
                    @test num_sets(s) == T(10)
                    @test num_sets(s2) == T(10)

                    for i = 1:10
                        @test find_root!(s, T(i)) == T(i)
                        @test set_size(s, T(i)) == one(T)
                    end
                ...
                @testset "union!" begin
                    ...
                    @test in_same_set(s, T(2), T(3))
                    @test find_root!(s, T(3)) == T(2)
                    @test set_size(s, T(2)) == T(2)
                    @test set_size(s, T(3)) == T(2)
                end
                ...
                @testset "more tests" begin
                    ...
                    @test set_size(s, T(8)) == T(4)
                    @test set_size(s, T(6)) == T(4)
                    union!(s, T(2), T(6))
                    @test find_root!(s, T(2)) == T(8)
                    @test set_size(s, T(3)) == T(6)
    ...
    @testset "DisjointSet" begin
        ...
        @testset "basic tests" begin
            ...
            @test length(empty(s)) == num_sets(empty(s)) == 0
        ...
        @testset "union!" begin
            for i = 1:5
                ...
                @test set_size(s, x) == set_size(s, y) == 2
            end
        ...
        @testset "root_union!" begin
            ...
            @test set_size(s, 7) == set_size(s, 3)
        end
        ...
        @testset "Some tests using non-integer disjoint sets" begin
            ...
            @test set_size(a, "a") == set_size(a, "b") == 2
