...
export IntDisjointSet, DisjointSet, find_root!, in_same_set, num_sets, root_union!, set_size
...
IntDisjointSet(n::T) where {T<:Integer} = IntDisjointSet{T}(collect(Base.OneTo(n)), ones(T, n), n)
IntDisjointSet{T}(n::Integer) where {T<:Integer} = IntDisjointSet{T}(collect(Base.OneTo(T(n))), ones(T, T(n)), T(n))
Base.length(s::IntDisjointSet) = length(s.parents)

"""
    num_sets(s::IntDisjointSet)
Get a number of sets.
"""
num_sets(s::IntDisjointSet) = s.nsets
...
function root_union!(s::IntDisjointSet{T}, x::T, y::T) where {T<:Integer}
    parents = s.parents
    size = s.sizes
    @inbounds xsize = size[x]
    @inbounds ysize = size[y]
...
"""
    set_size(s::IntDisjointSet{T}, x::T)
Returns the size of the set that `x` is in.
"""
function set_size(s::IntDisjointSet{T}, x::T) where {T<:Integer}
...
"""
    num_sets(s::DisjointSet)
Get the number of sets.
"""
num_sets(s::DisjointSet) = num_sets(s.internal)
...
"""
    set_size(s::DisjointSet{T}, x::T)
Returns the size of the set that `x` is in.
"""
set_size(s::DisjointSet{T}, x::T) where {T} = set_size(s.internal, s.intmap[x])
